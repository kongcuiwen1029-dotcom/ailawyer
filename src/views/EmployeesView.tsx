...
import { Bot, CheckCircle2, CircleDot, Link2, Plus, Power, ShieldAlert } from 'lucide-react'
          ...
          {employee.bindings.map(item => {
            ...
            return (
              <div className="wv-row" key={item.resourceId}>
                ...
                <div className="wv-row-actions">
                  <button className="wv-btn ghost danger" onClick={() => removeEmployeeBinding(employee.id, item.resourceId)}>解除</button>
                </div>
              </div>
            )
          })}
