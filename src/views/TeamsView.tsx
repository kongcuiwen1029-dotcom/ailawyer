...
import { AlertTriangle, Bot, CheckCircle2, CircleDot, Power, Plus, ShieldAlert, Users } from 'lucide-react'
...
export default function TeamsView({ onOpenEmployee, initialOpenId }: { onOpenEmployee: (employeeId: string) => void; initialOpenId?: string }) {
  ...
  if (open) return <TeamDetail key={open.id} team={open} onBack={() => setOpenId(null)} onOpenEmployee={onOpenEmployee} />
          ...
          return (
            <div className="wv-row" key={team.id} style={{ alignItems: 'flex-start' }}>
              ...
              <div className="wv-row-actions">
                ...
                {team.status === 'active'
                  ? <button className="wv-btn" onClick={() => disableTeam(team.id)}>停用</button>
...
function TeamDetail({ team, onBack, onOpenEmployee }: { team: Team; onBack: () => void; onOpenEmployee: (employeeId: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <Section title="预算" hint="超出预算会显式中止；主管不会在超限后自行重试。">
