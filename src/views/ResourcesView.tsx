...
export default function ResourcesView() {
  ...
  return (
    <>
      <ViewHead
        view="resources"
        meta="发布是可被绑定与执行的前提；取消发布不影响已引用版本的运行，但无法再被新员工绑定。"
        action={
          <>
        ...
        <div className="wv-list tight">
          {files.map(file => (
            <div className="wv-row" key={file.name}>
              ...
              <div className="wv-row-actions">
                <button className="wv-btn ghost danger" onClick={() => removeKnowledgeFile(resource.id, file.name)}><Trash2 size={12} strokeWidth={1.9} /> 删除</button>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <>
      ...
      <div className="wv-toolbar end">
        <button
          className="wv-btn"
          onClick={() => notify(blocked ? '试跑被阻止：请先修复校验错误。' : '已发起试跑，结果会写入调试台。', blocked ? 'warn' : 'ok')}
        >
