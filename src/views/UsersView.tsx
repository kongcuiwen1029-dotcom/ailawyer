...
import { Plus, Search, ShieldCheck, UserPlus, UserRound } from 'lucide-react'
    ...
    <>
      ...
      {tab === 'accounts' && (
        <>
          ...
          <table className="wv-table wv-freeze">
            <thead>
              <tr><th>用户</th><th>状态</th><th>角色</th><th>作用域</th><th>最近登录</th><th>最近活跃</th><th>会话</th><th>操作</th></tr>
            </thead>
            <tbody>
              {pageItems.map(user => (
                <tr key={user.id}>
                  <td>
                    <div className="wv-user">
                      <span className="wv-user-avatar"><UserRound size={15} strokeWidth={1.8} /></span>
                      <span className="wv-user-copy">
                        <span className="wv-user-name">
                          {user.name}
                          {user.id === CURRENT_USER_ID && <Badge kind="run">当前登录</Badge>}
                        </span>
                        <span className="wv-user-email">{user.email}</span>
                      </span>
                    </div>
                  </td>
                  <td>{user.status === 'active'
                    ? <Badge kind="ok">正常</Badge>
                    : <Badge kind="draft">已禁用</Badge>}</td>
                  <td>
                  ...
                  <td>
                    <div className="wv-row-actions">
                      ...
                      <button className="wv-btn ghost" onClick={() => resetUserPassword(user.id)}>重置密码</button>
                      <button className="wv-btn ghost" disabled={!user.sessions.length} onClick={() => revokeUserSessions(user.id)}>吊销会话</button>
