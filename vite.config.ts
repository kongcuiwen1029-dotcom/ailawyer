...
import { readSkin } from './tools/silk/load-skin.mjs'
...
/**
 * 让 `vite dev` 的页面也带上「丝绸背景」皮肤。
 *
 * 皮肤（tools/silk/skin.css + background.png）平时只由 tools/build-*.mjs 在构建时
 * 追加成第二个 <style>。结果是 `vite build` 的产物有丝绸、`vite dev`（也就是预览
 * 面板）没有 —— 灰色主题在那里只剩 index.css 自带的那条灰渐变，看着就跟背景被删了
 * 一样。这不是样式写错，是两个入口本来就没走同一条装配路径。
 *
 * 这里把构建用的同一个 readSkin 拿过来注入 dev 的 HTML，`apply: 'serve'` 保证只在
 * vite dev 生效：构建产物一个字节都不变，也不会出现两份 data URI。
 *
 * 注入到 body 末尾而不是 head：dev 下 index.css 是 Vite 在运行时 appendChild 到
 * head 的，位置排在所有静态标签之后；`.gy-page` 两边同优先级（0,1,0），谁在文档里
 * 靠后谁赢。放进 body，皮肤的规则才压得住 index.css 里那条灰色渐变。
 */
function silkSkinInDev(root: string): Plugin {
  return {
    name: 'silk-skin-in-dev',
    apply: 'serve',
    transformIndexHtml: {
      order: 'post',
      handler() {
        const { css } = readSkin(root)
        return [
          {
            tag: 'style',
            attrs: { 'data-silk-skin': '' },
            children: css,
            injectTo: 'body',
          },
        ]
    ...
    configureServer(server) {
      const { bytes } = readSkin(root)
      server.config.logger.info(`  silk skin  →  dev 页面已挂上丝绸背景（${(bytes / 1024).toFixed(0)} KB data URI）`)
    },
  }
}
